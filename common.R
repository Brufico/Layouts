...
columns <- function(n, aspect_ratio = 1, max_width = if (n == 1) 0.65 else 1) {
        ...
        # changed
        width <- doclinewidth / n * max_width # What was the 6 ? ... just the line width. changed: doclinewidth
...
hook_figdone <- 0 # = nombre de figures déjà imprimées sur la même ligne
...
plot_hook_bookdown <- function(x, options) {
        ...
        hook_figdone <- hook_figdone + 1 # nouveau
        message(hook_figdone) # nouveau (debug)
}
