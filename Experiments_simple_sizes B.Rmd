...
```{r basic, cache = TRUE, fig.width=w.12, fig.asp=a.34 } 
df <- data.frame(
  x = c(3, 1, 5), 
  y = c(2, 4, 6), 
  label = c("a","b","c")
...
```{r basic2, cache = TRUE, opts.label = ".f13"}
p <- ggplot(df, aes(x, y, label = label)) + 
  labs(x = NULL, y = NULL) + # Hide axis label
  theme(plot.title = element_text(size = 12)) # Shrink plot title
p + geom_point() + ggtitle("point")
...
```{r basicx2, cache = TRUE, w=w.14, fig.asp=a.13}
...
```{r pressure, echo=TRUE, w=w.13, a=a.12}
x1 = 1:6
y1 = sqrt(x1)
y2 = 1 - (x1 - 3) ^ 2 / 9
